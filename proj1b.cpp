...
void runOnWindow(int W1, int H1, int W2, int H2, Mat inputImage, char *outName) {
	...
	// get the a_min and b_max from the specific window 
	double a_min = 100;
	double b_max = 0;
	for (int i = H1; i <= H2; i++) {
		for (int j = W1; j <= W2; j++) {
	...
	// a_min = 0.254389, b_max = 91.5044
//    cout << a_min << ", " << b_max << "\n";

	// linear scaling in the whole picture 
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			long double r = R[i][j];
			long double g = G[i][j];
	...
	//		cout << L << ", ";
			...
			// Linear scaling in teh Luv domain
